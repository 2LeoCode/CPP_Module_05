...
/*   Updated: 2021/01/26 17:13:44 by lsuardi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int main(void)
{
	...
	OfficeBlock off(&newIntern, &paul, &jack);

	off.doBureaucracy("foo", "bar");
	off.doBureaucracy("presidential pardon", "Bob");
	off.doBureaucracy("robotomy request", "Bob");
	off.doBureaucracy("shrubbery creation", "tree3");

	Intern * someIntern = new SomeIntern;

	std::cout << "NEW INTERN" << std::endl << std::endl;
	off.setIntern(someIntern);
	off.doBureaucracy("shrubbery creation", "tree3");
	off.doBureaucracy("mutant pig termination", "Pigley");
	off.doBureaucracy("useless", "someone");
	off.doBureaucracy("promote", "Bob");
	delete someIntern;
	delete pre;
	delete rob;
	delete shrub;
